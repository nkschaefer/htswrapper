...
void* kmer_tree_lookup( const char* kmer, const kmer_tree_p ks, int rc) {
    size_t inx, kmer_pos;
    if ( kmer2inx( kmer, ks->k_ar_size, &inx, rc, ks->k ) ){
        return kmer_tree_lookup_aux(kmer, ks, rc, &inx);
    }
    else{
        return NULL;
    }
...
/**
 * Same as above, but continues on from a previous lookup, when scanning along
 * a sequence one base at a time.
 */
void* kmer_tree_lookup_continue( const char* kmer, const kmer_tree_p ks, int rc, size_t* inx) {
    // Shift inx by one base
    if (kmer2inx_continue( kmer, ks->k_ar_size, inx, rc, ks->k) ){
        return kmer_tree_lookup_aux(kmer, ks, rc, inx);
    }
    else{
        return NULL;
...
/**
 * For scanning along a sequence one base at a time, but corresponds to the first
 * lookup where idx is initialized.
 */
void* kmer_tree_lookup_init( const char* kmer, const kmer_tree_p ks, int rc, size_t* inx){
    // Build inx from scratch.
    if ( kmer2inx( kmer, ks->k_ar_size, inx, rc, ks->k ) ){
        return kmer_tree_lookup_aux(kmer, ks, rc, inx);
    }
    else{
...
void* kmer_tree_lookup_aux(const char* kmer, const kmer_tree_p ks, int rc, size_t* inx){
    size_t kmer_pos;
    ktnP curr_node = ks->lookup_array[*inx];
    char curr_base;

    /* If curr_node == NULL, we've never seen this kmer before.  */
    if ( curr_node == NULL ) {
        return NULL;
    }
...
/**
 * Same as above function, but for scanning through a sequence one k-mer
 * at a time.
 * Assumes that inx has already been constructed and we are just moving
 * ahead by one base in the sequence.
 */
int kmer2inx_continue( const char* kmer,
    const size_t kmer_len,
    size_t* inx,
    int rc,
    int k ) {
    
    size_t mask = (1UL << (kmer_len-1)*2) - 1;
    (*inx) &= mask;
    (*inx) = (*inx) << 2;

    if (rc){
        switch(toupper(kmer[k-kmer_len])){
            case 'T':
                (*inx) += 0;
                break;
            case 'G':
        ...
        switch(toupper(kmer[kmer_len-1])){
            case 'A':
                (*inx) += 0;
                break;
            case 'C':
